"""Attendance Validation Agent: gate write actions and set route."""
...
def attendance_validation_agent(state: WorkflowState) -> dict[str, Any]:
    decision = state.get("decision") or {}
    policy_results = state.get("policy_results") or {}
    analysis = state.get("analysis_results") or {}
    issues: list[str] = []

    _, notes_patch = search_short_term(state, agent="attendance_validation")
    ...
    if outcome == "approve":
        if severity not in {"warning"}:
            issues.append("Approve/warning actions require policy severity=warning.")
    ...
    if outcome == "blocked":
        ...
        if not (
            ...
            # Allow blocked with empty blockers only when employee missing entirely.
    ...
    # Authority invariant: memory/knowledge cannot clear policy violations into approve.
